...
"content" est TOUJOURS du texte brut, JAMAIS de Markdown (pas de "**gras**", pas de "# titre", pas de "- puce" en début de ligne) : ce texte est inséré tel quel dans un document Word sans interprétation, toute syntaxe Markdown y apparaîtrait littéralement (astérisques compris). Pour distinguer une sous-étape à l'intérieur d'un "content", termine la phrase qui l'introduit par un ":" puis va à la ligne — jamais un pseudo-titre en gras.`;
...
"corrected_content" est le texte complet et directement utilisable de la section corrigée — jamais un commentaire sur ce qui a été changé, jamais un texte générique déconnecté du contexte réel de cette procédure. Texte brut, JAMAIS de Markdown (pas de "**gras**", pas de "# titre") : inséré tel quel dans un document Word sans interprétation.`;
...
section_key est TOUJOURS la valeur "key" de la section (ex. "objectif"), jamais son libellé affiché (ex. "Objectif"). "rationale" : pourquoi cette CAPA justifie (ou non) une révision de la procédure (2 à 3 phrases). "suggested_changes" : une entrée par section à modifier — "current_excerpt" cite brièvement ce qui pose problème dans le texte actuel (chaîne vide si la section est absente), "suggested_content" est le nouveau texte proposé pour cette section, en texte brut, JAMAIS de Markdown (pas de "**gras**", pas de "# titre") : inséré tel quel dans un document Word sans interprétation. Ne propose de changement QUE si la CAPA le justifie réellement — un tableau vide est une réponse valide si la procédure n'a pas besoin d'évoluer.`;
