...
// les tendances affichées. Un tenant en échec n'empêche jamais les suivants : chacun est isolé
// dans son propre try/catch, comme ce job doit rester best-effort (une tendance manquante un
// jour n'est jamais aussi grave qu'une sauvegarde manquée). Exportée (même convention que les
// autres jobs, voir notificationJob.js) pour pouvoir être appelée manuellement :
// node -e "import('./src/jobs/dashboardSnapshotJob.js').then(m => m.runDashboardSnapshotJob())"
export async function runDashboardSnapshotJob() {
  ...
  if (error || !tenants) throw new Error(`Impossible de récupérer la liste des tenants : ${error?.message || 'réponse vide'}`);
  ...
  return { ok: succeeded, total: tenants.length };
}

// Après la sauvegarde (2h) pour ne jamais les faire concourir pour les mêmes ressources DB.
export function scheduleDashboardSnapshotJob() {
  cron.schedule('0 3 * * *', () => {
    withJobRunTracking('dashboardSnapshotJob', runDashboardSnapshotJob).catch((err) =>
      console.error('[dashboardSnapshotJob] Échec :', err.message)
    );
  });
